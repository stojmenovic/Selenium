...
public class DBBanka {
    ...
    public void printBankOffices() throws SQLException {

        try (Connection connection = DriverManager.getConnection(connectionString);
             Statement statement = connection.createStatement()) {
        	final ResultSet resultSet = statement.executeQuery("SELECT f.naziv, f.IdMes, m.nazivmesta FROM Filijala f, Mesto m WHERE f.IDMes=m.IDMes");
            // statement.executeQuery koristiti samo za SELECT upite
            ...
            while (resultSet.next()) {  // dok postoji red za citanje -> ucitaj
                ...
                // Iz ucitanog reda preuzmi kolone
                // Paznja: Indeksi kolona se broje od 1 (ne od 0)
            	//int idfil= resultSet.getInt(2);
            	String nazivfil = resultSet.getString("Naziv");
            	//int idmes= resultSet.getInt(1);
            	String nazivmes = resultSet.getString("NazivMesta");
            	
                
               
                System.out.println(nazivfil + "\t" + nazivmes);}}
            }
